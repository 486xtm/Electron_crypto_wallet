...
export default {
	initialize() {
		...
		ipcMain.on(ipcChannels.MINIMIZE, (_event: any) => {
			windows.mainWindow?.minimize();
		});

		ipcMain.on(ipcChannels.MAXIMIZE, (_event: any) => {
			console.log(
				'ok',
			...
			} else windows.mainWindow?.maximize();
		});

		ipcMain.on(ipcChannels.CLOSE, (_event: any) => {
